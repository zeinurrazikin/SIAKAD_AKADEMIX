        ...
        <div class="relative mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
            <div class="min-w-0 flex-1">
                ...
                <p class="mt-3 text-cyan-100">
                    Perbarui data untuk ruangan: <span class="font-bold">{{ $ruangan->nama_ruangan }}</span>
                </p>
            </div>
        </div>
    </div>
                ...
                <form action="{{ route('admin.ruangan.update', $ruangan->id) }}" method="POST">
                    ...
                    <div class="p-8 space-y-6">
                        <div>
                            ...
                            <input type="text" name="kode_ruangan" id="kode_ruangan" value="{{ old('kode_ruangan', $ruangan->kode_ruangan) }}" class="block w-full px-4 py-3 border border-gray-200 rounded-xl shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200" required>
                        </div>

                        <div>
                            <label for="nama_ruangan" class="block text-sm font-semibold text-gray-700 mb-1">Nama Ruangan / Keterangan</label>
                            <input type="text" name="nama_ruangan" id="nama_ruangan" value="{{ old('nama_ruangan', $ruangan->nama_ruangan) }}" class="block w-full px-4 py-3 border border-gray-200 rounded-xl shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200" required>
                        </div>
                    </div>
