        ...
        <div class="relative mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
            <div class="min-w-0 flex-1">
                ...
                <p class="mt-3 text-cyan-100">
                    Kelola daftar ruangan kelas dan laboratorium yang tersedia.
                </p>
            </div>
        </div>
    </div>
    ...
    <main class="-mt-24 pb-12">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('admin.ruangan.create') }}" class="inline-flex items-center gap-2 rounded-md bg-indigo-600/90 backdrop-blur-sm px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700 border border-indigo-500/50 transition-all">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div class="overflow-x-auto border border-gray-200 rounded-lg">
                        <table class="w-full text-sm">
                            <thead class="text-left text-xs uppercase tracking-wider text-gray-600 bg-gray-100">
                                <tr>
                                    <th class="p-4 font-bold">Kode Ruangan</th>
                                    <th class="p-4 font-bold">Nama Ruangan</th>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="p-4 font-mono text-gray-800">{{ $r->kode_ruangan }}</td>
                                    <td class="p-4 font-semibold text-gray-800">{{ $r->nama_ruangan }}</td>
                                    <td class="p-4 text-right space-x-2">
                                        <a href="{{ route('admin.ruangan.edit', $r->id) }}" class="text-white bg-yellow-500 hover:bg-yellow-600 px-3 py-1 rounded text-xs font-bold transition">Edit</a>
                                        <form action="{{ route('admin.ruangan.destroy', $r->id) }}" method="POST" class="inline" onsubmit="return confirm('Anda yakin ingin menghapus ruangan ini?')">
                                            @csrf @method('DELETE')
