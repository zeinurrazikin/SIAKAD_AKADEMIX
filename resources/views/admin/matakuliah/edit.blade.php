        ...
        <div class="relative mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
            <div class="min-w-0 flex-1">
                ...
                <p class="mt-3 text-yellow-100">
                    Perbarui data untuk mata kuliah: <span class="font-bold">{{ $mataKuliah->nama_mk }}</span>
                </p>
            </div>
        </div>
    </div>
                ...
                <form action="{{ route('admin.matakuliah.update', $mataKuliah->id) }}" method="POST">
                    ...
                    <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="nama_mk" id="nama_mk" value="{{ old('nama_mk', $mataKuliah->nama_mk) }}" class="block w-full px-4 py-3 border border-gray-200 rounded-xl shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200" required>
                        </div>
                        <div>
                            <label for="kode_mk" class="block text-sm font-semibold text-gray-700 mb-1">Kode MK</label>
                            <input type="text" name="kode_mk" id="kode_mk" value="{{ old('kode_mk', $mataKuliah->kode_mk) }}" class="block w-full px-4 py-3 border border-gray-200 rounded-xl shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200" required>
                        </div>

                        <div>
                            <label for="sks" class="block text-sm font-semibold text-gray-700 mb-1">Jumlah SKS</label>
                            <input type="number" name="sks" id="sks" value="{{ old('sks', $mataKuliah->sks) }}" class="block w-full px-4 py-3 border border-gray-200 rounded-xl shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200" required>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="semester_paket" id="semester_paket" value="{{ old('semester_paket', $mataKuliah->semester_paket) }}" class="block w-full px-4 py-3 border border-gray-200 rounded-xl shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200" required>
